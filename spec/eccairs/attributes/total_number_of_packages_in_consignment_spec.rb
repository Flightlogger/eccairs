...
RSpec.describe Eccairs::Attributes::TotalNumberOfPackagesInConsignment do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1054")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Total_Number_Of_Packages_In_Consignment")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_dangerous_goods do |dg|
          dg.add_total_number_of_packages_in_consignment(10)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Total_Number_Of_Packages_In_Consignment")
      expect(xml).to include('attributeId="1054"')
    end
