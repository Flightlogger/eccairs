...
RSpec.describe Eccairs::Attributes::TypeOfGroundStaff do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1117")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Type_of_Ground_staff")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_ground_handling do |gh|
          gh.add_type_of_ground_staff("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Type_of_Ground_staff")
      expect(xml).to include('attributeId="1117"')
    end
