...
RSpec.describe Eccairs::Attributes::DateOfInspection do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Date_Of_Inspection")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_engine do |engine|
            engine.add_date_of_inspection("2024-01-01")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Date_Of_Inspection")
      expect(xml).to include('attributeId="890"')
    end
