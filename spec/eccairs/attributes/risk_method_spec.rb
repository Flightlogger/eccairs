...
RSpec.describe Eccairs::Attributes::RiskMethod do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Risk_Method")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_risk_assessment do |ra|
          ra.add_risk_method("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Risk_Method")
      expect(xml).to include('attributeId="939"')
    end
