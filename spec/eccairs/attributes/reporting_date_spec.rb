...
RSpec.describe Eccairs::Attributes::ReportingDate do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Reporting_Date")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_reporting_history do |rh|
          rh.add_reporting_date("2024-01-01")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Reporting_Date")
      expect(xml).to include('attributeId="801"')
    end
