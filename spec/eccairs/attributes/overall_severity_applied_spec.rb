...
RSpec.describe Eccairs::Attributes::OverallSeverityApplied do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1076")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Overall_Severity_Applied")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_overall_severity_applied("1")
      end

      xml = set.to_xml
      expect(xml).to include("Overall_Severity_Applied")
      expect(xml).to include('attributeId="1076"')
    end
