...
RSpec.describe Eccairs::Attributes::TurbulenceIntensity do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("293")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Turbulence_Intensity")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_turbulence_intensity("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Turbulence_Intensity")
      expect(xml).to include('attributeId="293"')
    end
