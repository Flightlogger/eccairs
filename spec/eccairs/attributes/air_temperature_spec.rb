...
RSpec.describe Eccairs::Attributes::AirTemperature do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("287")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Air_Temperature")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_temperature(10.5)
      end

      xml = set.to_xml
      expect(xml).to include("Air_Temperature")
      expect(xml).to include('attributeId="287"')
    end
