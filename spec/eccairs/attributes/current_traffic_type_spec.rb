...
RSpec.describe Eccairs::Attributes::CurrentTrafficType do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Current_Traffic_Type")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_current_traffic_type("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Current_Traffic_Type")
      expect(xml).to include('attributeId="29"')
    end
