...
RSpec.describe Eccairs::Attributes::LocationOnGround do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1118")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Location_On_Ground")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_ground_handling do |gh|
          gh.add_location_on_ground("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Location_On_Ground")
      expect(xml).to include('attributeId="1118"')
    end
