...
RSpec.describe Eccairs::Attributes::Manufacturer do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Manufacturer")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_part_information do |part|
            part.add_manufacturer("1")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Manufacturer")
      expect(xml).to include('attributeId="658"')
    end
