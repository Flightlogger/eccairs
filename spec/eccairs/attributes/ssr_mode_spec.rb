...
RSpec.describe Eccairs::Attributes::SsrMode do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("SSR_Mode")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_ssr_mode("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("SSR_Mode")
      expect(xml).to include('attributeId="278"')
    end
