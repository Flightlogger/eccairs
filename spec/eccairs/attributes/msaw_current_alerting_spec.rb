...
RSpec.describe Eccairs::Attributes::MsawCurrentAlerting do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("369")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("MSAW_Current_Alerting")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_navigation_service do |ans|
          ans.add_msaw_current_alerting("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("MSAW_Current_Alerting")
      expect(xml).to include('attributeId="369"')
    end
