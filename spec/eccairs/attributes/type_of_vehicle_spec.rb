...
RSpec.describe Eccairs::Attributes::TypeOfVehicle do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Type_Of_Vehicle")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aerodrome_general do |aerodrome|
          aerodrome.add_vehicle do |vehicle|
            vehicle.add_type_of_vehicle("1")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Type_Of_Vehicle")
      expect(xml).to include('attributeId="733"')
    end
