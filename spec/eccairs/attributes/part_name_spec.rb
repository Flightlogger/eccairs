...
RSpec.describe Eccairs::Attributes::PartName do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Part_Name")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_part_information do |part|
            part.add_part_name("test value")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Part_Name")
      expect(xml).to include('attributeId="485"')
    end
