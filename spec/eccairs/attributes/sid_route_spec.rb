...
RSpec.describe Eccairs::Attributes::SidRoute do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("264")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("SID_Route")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_sid_route("test value")
        end
      end

      xml = set.to_xml
      expect(xml).to include("SID_Route")
      expect(xml).to include('attributeId="264"')
    end
