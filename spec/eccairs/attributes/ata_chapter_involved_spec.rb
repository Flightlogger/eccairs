...
RSpec.describe Eccairs::Attributes::AtaChapterInvolved do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("ATA_Chapter_Involved")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_ata_chapter_involved("11000000")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("ATA_Chapter_Involved")
      expect(xml).to include('attributeId="894"')
    end
