...
RSpec.describe Eccairs::Attributes::TimeDeviatingFl do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Time_Deviating_FL")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_time_deviating_fl(10.5)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Time_Deviating_FL")
      expect(xml).to include('attributeId="816"')
    end
