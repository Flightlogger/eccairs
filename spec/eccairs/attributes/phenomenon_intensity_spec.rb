...
RSpec.describe Eccairs::Attributes::PhenomenonIntensity do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("230")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Phenomenon_Intensity")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_precipitation_and_other_weather_phenomena do |weather|
          weather.add_phenomenon_intensity("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Phenomenon_Intensity")
      expect(xml).to include('attributeId="230"')
    end
