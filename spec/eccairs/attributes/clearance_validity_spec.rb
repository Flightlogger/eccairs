...
RSpec.describe Eccairs::Attributes::ClearanceValidity do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("57")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Clearance_Validity")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_clearance_validity("2")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Clearance_Validity")
      expect(xml).to include('attributeId="57"')
    end
