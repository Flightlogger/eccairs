...
RSpec.describe Eccairs::Attributes::HeightOfCloudBase do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("140")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Height_Of_Cloud_Base")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_height_of_cloud_base(10.5)
      end

      xml = set.to_xml
      expect(xml).to include("Height_Of_Cloud_Base")
      expect(xml).to include('attributeId="140"')
    end
