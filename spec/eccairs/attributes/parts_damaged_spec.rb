...
RSpec.describe Eccairs::Attributes::PartsDamaged do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("643")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Parts_Damaged")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_parts_damaged("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Parts_Damaged")
      expect(xml).to include('attributeId="643"')
    end
