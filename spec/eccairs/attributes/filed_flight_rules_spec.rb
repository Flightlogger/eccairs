...
RSpec.describe Eccairs::Attributes::FiledFlightRules do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("117")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Filed_Flight_Rules")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_filed_flight_rules("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Filed_Flight_Rules")
      expect(xml).to include('attributeId="117"')
    end
