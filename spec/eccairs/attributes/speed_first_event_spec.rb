...
RSpec.describe Eccairs::Attributes::SpeedFirstEvent do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Speed_First_Event")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_speed_first_event(10.5)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Speed_First_Event")
      expect(xml).to include('attributeId="273"')
    end
