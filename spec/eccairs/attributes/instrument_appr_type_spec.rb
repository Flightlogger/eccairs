...
RSpec.describe Eccairs::Attributes::InstrumentApprType do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("165")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Instrument_Appr_Type")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_instrument_appr_type("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Instrument_Appr_Type")
      expect(xml).to include('attributeId="165"')
    end
