...
RSpec.describe Eccairs::Attributes::TotalNumberSeriousInjuries do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("262")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Total_Number_Serious_Injuries")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_total_number_serious_injuries(10)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Total_Number_Serious_Injuries")
      expect(xml).to include('attributeId="262"')
    end
