...
RSpec.describe Eccairs::Attributes::LandingAreaType do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("9")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Landing_Area_Type")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aerodrome_general do |aerodrome|
          aerodrome.add_landing_area_type("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Landing_Area_Type")
      expect(xml).to include('attributeId="9"')
    end
