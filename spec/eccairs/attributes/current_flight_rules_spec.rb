...
RSpec.describe Eccairs::Attributes::CurrentFlightRules do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("79")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Current_Flight_Rules")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_current_flight_rules("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Current_Flight_Rules")
      expect(xml).to include('attributeId="79"')
    end
