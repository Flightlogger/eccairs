...
RSpec.describe Eccairs::Attributes::OriginOfGoods do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Origin_Of_Goods")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_dangerous_goods do |dg|
          dg.add_origin_of_goods("test value")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Origin_Of_Goods")
      expect(xml).to include('attributeId="1051"')
    end
