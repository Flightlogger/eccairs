...
RSpec.describe Eccairs::Attributes::SpecTechRepeatability do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1081")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Spec_Tech_Repeatability")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_spec_tech_repeatability("1")
      end

      xml = set.to_xml
      expect(xml).to include("Spec_Tech_Repeatability")
      expect(xml).to include('attributeId="1081"')
    end
