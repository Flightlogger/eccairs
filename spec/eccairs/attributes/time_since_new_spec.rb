...
RSpec.describe Eccairs::Attributes::TimeSinceNew do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Time_Since_New")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_time_since_new(10.5)
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Time_Since_New")
      expect(xml).to include('attributeId="899"')
    end
