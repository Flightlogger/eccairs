...
RSpec.describe Eccairs::Attributes::ShipperName do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Shipper_Name")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_dangerous_goods do |dg|
          dg.add_shipper_name("test value")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Shipper_Name")
      expect(xml).to include('attributeId="1053"')
    end
