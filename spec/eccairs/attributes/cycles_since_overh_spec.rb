...
RSpec.describe Eccairs::Attributes::CyclesSinceOverh do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Cycles_Since_Overh")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_cycles_since_overh(10)
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Cycles_Since_Overh")
      expect(xml).to include('attributeId="903"')
    end
