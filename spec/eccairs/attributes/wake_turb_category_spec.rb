...
RSpec.describe Eccairs::Attributes::WakeTurbCategory do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("313")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Wake_Turb_Category")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_wake_turb_category("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Wake_Turb_Category")
      expect(xml).to include('attributeId="313"')
    end
