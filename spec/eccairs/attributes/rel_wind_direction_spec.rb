...
RSpec.describe Eccairs::Attributes::RelWindDirection do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("245")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Rel_Wind_Direction")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_rel_wind_direction("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Rel_Wind_Direction")
      expect(xml).to include('attributeId="245"')
    end
