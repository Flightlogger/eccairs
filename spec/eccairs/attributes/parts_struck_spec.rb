...
RSpec.describe Eccairs::Attributes::PartsStruck do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("644")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Parts_Struck")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_parts_struck("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Parts_Struck")
      expect(xml).to include('attributeId="644"')
    end
