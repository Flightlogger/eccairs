...
RSpec.describe Eccairs::Attributes::MakeOfPropeller do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Make_Of_Propeller")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_make_of_propeller("1")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Make_Of_Propeller")
      expect(xml).to include('attributeId="492"')
    end
