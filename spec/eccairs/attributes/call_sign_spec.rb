...
RSpec.describe Eccairs::Attributes::CallSign do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Call_Sign")
    end
  ...
  describe "initialization" do
    it "creates an instance with a value" do
      instance = described_class.new("ABC123")
      expect(instance.value).to eq("ABC123")
    end
  end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_call_sign("ABC123")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Call_Sign")
      expect(xml).to include('attributeId="54"')
    end
