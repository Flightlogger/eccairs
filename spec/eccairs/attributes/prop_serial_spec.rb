...
RSpec.describe Eccairs::Attributes::PropSerial do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("893")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Prop_Serial")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_prop_serial("test value")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Prop_Serial")
      expect(xml).to include('attributeId="893"')
    end
