...
RSpec.describe Eccairs::Attributes::OccOnGround do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("213")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Occ_On_Ground")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_occ_on_ground("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Occ_On_Ground")
      expect(xml).to include('attributeId="213"')
    end
