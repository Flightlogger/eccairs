...
RSpec.describe Eccairs::Attributes::LongitudeOfOcc do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("444")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Longitude_Of_Occ")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_longitude_of_occ(10.5)
      end

      xml = set.to_xml
      expect(xml).to include("Longitude_Of_Occ")
      expect(xml).to include('attributeId="444"')
    end
