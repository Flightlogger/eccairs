...
RSpec.describe Eccairs::Attributes::ControllingAgency do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("64")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Controlling_Agency")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_controlling_agency("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Controlling_Agency")
      expect(xml).to include('attributeId="64"')
    end
