...
RSpec.describe Eccairs::Attributes::DateOfManufacturing do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Date_Of_Manufacturing")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_date_of_manufacturing("2024-01-01")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Date_Of_Manufacturing")
      expect(xml).to include('attributeId="905"')
    end
