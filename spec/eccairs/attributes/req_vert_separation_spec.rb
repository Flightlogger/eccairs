...
RSpec.describe Eccairs::Attributes::ReqVertSeparation do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("583")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Req_Vert_Separation")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_separation do |sep|
          sep.add_req_vert_separation(10.5)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Req_Vert_Separation")
      expect(xml).to include('attributeId="583"')
    end
