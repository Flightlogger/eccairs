...
RSpec.describe Eccairs::Attributes::RiskGrade do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("718")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Risk_Grade")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Risk_Grade")
      expect(xml).to include('attributeId="718"')
    end
