...
RSpec.describe Eccairs::Attributes::AircraftCategory do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("32")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Aircraft_Category")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Aircraft_Category")
      expect(xml).to include('attributeId="32"')
    end
