...
RSpec.describe Eccairs::Attributes::AreaConfiguration do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Area_Configuration")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aerodrome_general do |aerodrome|
          aerodrome.add_area_configuration("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Area_Configuration")
      expect(xml).to include('attributeId="3"')
    end
