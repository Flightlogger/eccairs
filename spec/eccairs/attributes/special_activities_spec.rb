...
RSpec.describe Eccairs::Attributes::SpecialActivities do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Special_Activities")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_space do |as|
          as.add_special_activities("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Special_Activities")
      expect(xml).to include('attributeId="17"')
    end
