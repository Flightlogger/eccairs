...
RSpec.describe Eccairs::Attributes::RunwayNumber do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Runway_Number")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aerodrome_general do |aerodrome|
          aerodrome.add_runway do |runway|
            runway.add_runway_number("1")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Runway_Number")
      expect(xml).to include('attributeId="1119"')
    end
