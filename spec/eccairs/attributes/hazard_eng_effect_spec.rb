...
RSpec.describe Eccairs::Attributes::HazardEngEffect do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("945")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Hazard_Eng_Effect")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Hazard_Eng_Effect")
      expect(xml).to include('attributeId="945"')
    end
