...
RSpec.describe Eccairs::Attributes::FlightPlanType do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Flight_Plan_Type")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_flight_plan_type("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Flight_Plan_Type")
      expect(xml).to include('attributeId="297"')
    end
