# frozen_string_literal: true

require "spec_helper"

RSpec.describe Eccairs::Attributes::UtcDate do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("477")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("UTC_Date")
    end

    it "inherits from DateAttribute" do
      expect(described_class.superclass).to eq(Eccairs::Base::DateAttribute)
    end
  end
  
  describe "initialization" do
    it "creates an instance with a value" do
      instance = described_class.new("2024-01-01")
      expect(instance.value).to eq("2024-01-01")
    end
  end
  
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      set = Eccairs.set
      set.add_occurrence do |occurrence|
        occurrence.add_utc_date("2024-01-01")
      end

      xml = set.to_xml
      expect(xml).to include("UTC_Date")
      expect(xml).to include('attributeId="477"')
    end

    it "validates successfully in a minimal occurrence" do
      set = Eccairs.set
      set.add_occurrence do |occurrence|
        occurrence.add_utc_date("2024-01-01")
      end

      errors = set.validate
      expect(errors).to be_empty, "Expected no validation errors, got: #{errors.map(&:message).join(', ')}"
    end
  end
end
