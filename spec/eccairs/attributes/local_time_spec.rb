# frozen_string_literal: true

require "spec_helper"

RSpec.describe Eccairs::Attributes::LocalTime do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("457")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Local_Time")
    end

    it "inherits from TimeAttribute" do
      expect(described_class.superclass).to eq(Eccairs::Base::TimeAttribute)
    end
  end

  describe "initialization" do
    it "creates an instance with a value" do
      instance = described_class.new("12:00:00")
      expect(instance.value).to eq("12:00:00")
    end
  end

  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      set = Eccairs.set
      set.add_occurrence do |occurrence|
        occurrence.add_local_time("12:00:00")
      end

      xml = set.to_xml
      expect(xml).to include("Local_Time")
      expect(xml).to include('attributeId="457"')
    end

    it "validates successfully in a minimal occurrence" do
      set = Eccairs.set
      set.add_occurrence do |occurrence|
        occurrence.add_local_time("12:00:00")
      end

      errors = set.validate
      expect(errors).to be_empty, "Expected no validation errors, got: #{errors.map(&:message).join(", ")}"
    end
  end
end
