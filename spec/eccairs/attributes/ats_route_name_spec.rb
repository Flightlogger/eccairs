...
RSpec.describe Eccairs::Attributes::AtsRouteName do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("46")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("ATS_Route_Name")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_ats_route_name("test value")
        end
      end

      xml = set.to_xml
      expect(xml).to include("ATS_Route_Name")
      expect(xml).to include('attributeId="46"')
    end
