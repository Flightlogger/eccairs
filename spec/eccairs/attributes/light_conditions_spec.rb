...
RSpec.describe Eccairs::Attributes::LightConditions do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("168")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Light_Conditions")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_light_conditions("1")
      end

      xml = set.to_xml
      expect(xml).to include("Light_Conditions")
      expect(xml).to include('attributeId="168"')
    end
