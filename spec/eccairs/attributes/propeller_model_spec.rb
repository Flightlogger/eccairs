...
RSpec.describe Eccairs::Attributes::PropellerModel do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Propeller_Model")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_propeller_model("1")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Propeller_Model")
      expect(xml).to include('attributeId="493"')
    end
