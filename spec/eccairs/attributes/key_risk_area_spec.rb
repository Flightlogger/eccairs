...
RSpec.describe Eccairs::Attributes::KeyRiskArea do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1097")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Key_Risk_Area")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_risk_assessment do |ra|
          ra.add_key_risk_area("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Key_Risk_Area")
      expect(xml).to include('attributeId="1097"')
    end
