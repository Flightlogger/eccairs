...
RSpec.describe Eccairs::Attributes::LandingGearType do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("166")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Landing_Gear_Type")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_landing_gear_type("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Landing_Gear_Type")
      expect(xml).to include('attributeId="166"')
    end
