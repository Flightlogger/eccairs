...
RSpec.describe Eccairs::Attributes::DangGoodsInvolved do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Dang_Goods_Involved")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_dang_goods_involved("1")
      end

      xml = set.to_xml
      expect(xml).to include("Dang_Goods_Involved")
      expect(xml).to include('attributeId="129"')
    end
