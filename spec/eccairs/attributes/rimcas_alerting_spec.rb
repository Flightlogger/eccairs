...
RSpec.describe Eccairs::Attributes::RimcasAlerting do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("366")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("RIMCAS_Alerting")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_navigation_service do |ans|
          ans.add_rimcas_alerting("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("RIMCAS_Alerting")
      expect(xml).to include('attributeId="366"')
    end
