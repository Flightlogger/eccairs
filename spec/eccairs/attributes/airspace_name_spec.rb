...
RSpec.describe Eccairs::Attributes::AirspaceName do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Airspace_Name")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_space do |as|
          as.add_airspace_name("test value")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Airspace_Name")
      expect(xml).to include('attributeId="14"')
    end
