...
RSpec.describe Eccairs::Attributes::OverallRepeatability do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1078")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Overall_Repeatability")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_overall_repeatability("1")
      end

      xml = set.to_xml
      expect(xml).to include("Overall_Repeatability")
      expect(xml).to include('attributeId="1078"')
    end
