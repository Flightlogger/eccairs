...
RSpec.describe Eccairs::Attributes::GhTypeOfService do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1115")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("GH_Type_Of_Service")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("GH_Type_Of_Service")
      expect(xml).to include('attributeId="1115"')
    end
