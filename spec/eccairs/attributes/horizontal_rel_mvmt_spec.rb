...
RSpec.describe Eccairs::Attributes::HorizontalRelMvmt do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("570")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Horizontal_Rel_Mvmt")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Horizontal_Rel_Mvmt")
      expect(xml).to include('attributeId="570"')
    end
