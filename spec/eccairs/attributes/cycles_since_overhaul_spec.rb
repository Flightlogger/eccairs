...
RSpec.describe Eccairs::Attributes::CyclesSinceOverhaul do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("664")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Cycles_Since_Overhaul")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_part_information do |part|
            part.add_cycles_since_overhaul(10)
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Cycles_Since_Overhaul")
      expect(xml).to include('attributeId="664"')
    end
