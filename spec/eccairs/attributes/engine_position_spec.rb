...
RSpec.describe Eccairs::Attributes::EnginePosition do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("653")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Engine_Position")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Engine_Position")
      expect(xml).to include('attributeId="653"')
    end
