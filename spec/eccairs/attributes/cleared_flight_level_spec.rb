...
RSpec.describe Eccairs::Attributes::ClearedFlightLevel do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Cleared_Flight_Level")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_cleared_flight_level(10)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Cleared_Flight_Level")
      expect(xml).to include('attributeId="60"')
    end
