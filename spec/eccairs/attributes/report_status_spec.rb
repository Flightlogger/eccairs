...
RSpec.describe Eccairs::Attributes::ReportStatus do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("800")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Report_Status")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_reporting_history do |rh|
          rh.add_report_status("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Report_Status")
      expect(xml).to include('attributeId="800"')
    end
