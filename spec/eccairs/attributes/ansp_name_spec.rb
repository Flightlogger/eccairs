...
RSpec.describe Eccairs::Attributes::AnspName do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("ANSP_Name")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_navigation_service do |ans|
          ans.add_ansp_name("test value")
        end
      end

      xml = set.to_xml
      expect(xml).to include("ANSP_Name")
      expect(xml).to include('attributeId="372"')
    end
