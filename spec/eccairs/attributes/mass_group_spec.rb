...
RSpec.describe Eccairs::Attributes::MassGroup do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("319")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Mass_Group")
    end
  ...
  describe "initialization" do
    it "creates an instance with a value" do
      instance = described_class.new("101")
      expect(instance.value).to eq(101)
    end
  end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_mass_group(101)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Mass_Group")
      expect(xml).to include('attributeId="319"')
    end
