...
RSpec.describe Eccairs::Attributes::RateOfClosure do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("588")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Rate_Of_Closure")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_separation do |sep|
          sep.add_rate_of_closure(10.5)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Rate_Of_Closure")
      expect(xml).to include('attributeId="588"')
    end
