...
RSpec.describe Eccairs::Attributes::HazardousPropEffects do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("946")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Hazardous_Prop_Effects")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_hazardous_prop_effects("1")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Hazardous_Prop_Effects")
      expect(xml).to include('attributeId="946"')
    end
