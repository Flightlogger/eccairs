...
RSpec.describe Eccairs::Attributes::ApwAlerting do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("363")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("APW_Alerting")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_navigation_service do |ans|
          ans.add_apw_alerting("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("APW_Alerting")
      expect(xml).to include('attributeId="363"')
    end
