...
RSpec.describe Eccairs::Attributes::BirdSize do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("648")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Bird_Size")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Bird_Size")
      expect(xml).to include('attributeId="648"')
    end
