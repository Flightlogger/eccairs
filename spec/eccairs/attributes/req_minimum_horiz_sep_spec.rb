...
RSpec.describe Eccairs::Attributes::ReqMinimumHorizSep do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("577")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Req_Minimum_Horiz_Sep")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_separation do |sep|
          sep.add_req_minimum_horiz_sep(10.5)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Req_Minimum_Horiz_Sep")
      expect(xml).to include('attributeId="577"')
    end
