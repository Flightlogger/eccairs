...
RSpec.describe Eccairs::Attributes::BirdsWildlifeStruck do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("647")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Birds_Wildlife_Struck")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Birds_Wildlife_Struck")
      expect(xml).to include('attributeId="647"')
    end
