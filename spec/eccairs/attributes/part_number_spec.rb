...
RSpec.describe Eccairs::Attributes::PartNumber do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Part_Number")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_part_information do |part|
            part.add_part_number("test value")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Part_Number")
      expect(xml).to include('attributeId="486"')
    end
