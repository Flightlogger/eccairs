...
RSpec.describe Eccairs::Attributes::FiledTrafficType do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("118")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Filed_Traffic_Type")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_filed_traffic_type("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Filed_Traffic_Type")
      expect(xml).to include('attributeId="118"')
    end
