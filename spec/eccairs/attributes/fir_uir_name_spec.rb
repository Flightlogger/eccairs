...
RSpec.describe Eccairs::Attributes::FirUirName do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("16")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("FIR_UIR_Name")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_air_space do |as|
          as.add_fir_uir_name("1000")
        end
      end

      xml = set.to_xml
      expect(xml).to include("FIR_UIR_Name")
      expect(xml).to include('attributeId="16"')
    end
