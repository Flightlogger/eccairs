...
RSpec.describe Eccairs::Attributes::TypeOfEquipmentInvolved do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1120")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Type_Of_Equipment_Involved")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_ground_handling do |gh|
          gh.add_type_of_equipment_involved("1")
        end
      end

      xml = set.to_xml
      expect(xml).to include("Type_Of_Equipment_Involved")
      expect(xml).to include('attributeId="1120"')
    end
