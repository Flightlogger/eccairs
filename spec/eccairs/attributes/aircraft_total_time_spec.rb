...
RSpec.describe Eccairs::Attributes::AircraftTotalTime do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("291")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Aircraft_Total_Time")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_aircraft_total_time(10.5)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Aircraft_Total_Time")
      expect(xml).to include('attributeId="291"')
    end
