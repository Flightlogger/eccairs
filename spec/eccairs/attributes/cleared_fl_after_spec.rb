...
RSpec.describe Eccairs::Attributes::ClearedFlAfter do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Cleared_FL_After")
    end
    ...
    it "generates valid XML within an occurrence" do
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_cleared_fl_after(10.5)
        end
      end

      xml = set.to_xml
      expect(xml).to include("Cleared_FL_After")
      expect(xml).to include('attributeId="817"')
    end
