...
RSpec.describe Eccairs::Attributes::CyclesSinceInsp do
  describe "class configuration" do
    ...
    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Cycles_Since_Insp")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_propeller do |prop|
            prop.add_cycles_since_insp(10)
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Cycles_Since_Insp")
      expect(xml).to include('attributeId="904"')
    end
