...
RSpec.describe Eccairs::Attributes::LifeControlledPart do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("1114")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Life_Controlled_Part")
    end
      ...
      set.add_occurrence do |occurrence|
        occurrence.add_aircraft do |aircraft|
          aircraft.add_part_information do |part|
            part.add_life_controlled_part("1")
          end
        end
      end

      xml = set.to_xml
      expect(xml).to include("Life_Controlled_Part")
      expect(xml).to include('attributeId="1114"')
    end
