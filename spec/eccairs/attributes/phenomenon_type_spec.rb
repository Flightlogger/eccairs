...
RSpec.describe Eccairs::Attributes::PhenomenonType do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("299")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Phenomenon_Type")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Phenomenon_Type")
      expect(xml).to include('attributeId="299"')
    end
