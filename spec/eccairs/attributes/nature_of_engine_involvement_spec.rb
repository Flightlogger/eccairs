...
RSpec.describe Eccairs::Attributes::NatureOfEngineInvolvement do
  describe "class configuration" do
    it "has correct attribute_id" do
      expect(described_class.attribute_id).to eq("654")
    end

    it "has correct xml_tag" do
      expect(described_class.xml_tag).to eq("Nature_Of_Engine_Involvement")
    end
  ...
  describe "XML generation in occurrence" do
    it "generates valid XML within an occurrence" do
      ...
      xml = set.to_xml
      expect(xml).to include("Nature_Of_Engine_Involvement")
      expect(xml).to include('attributeId="654"')
    end
