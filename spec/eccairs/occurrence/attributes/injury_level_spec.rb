...
RSpec.describe Eccairs::Occurrence::Attributes::InjuryLevel do
  ...
  describe ".attribute_id" do
    it "returns attribute ID of 451" do
      expect(described_class.attribute_id).to eq("451")
    end
  end
  ...
  describe "#build_xml" do
    it "generates valid XML with value" do
      ...
      expect(xml).to include("Injury_Level")
      expect(xml).to include('attributeId="451"')
    end
    ...
    it "does not generate XML when value is nil" do
      ...
      expect(xml).not_to include("Injury_Level")
    end
  end
