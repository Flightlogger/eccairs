...
RSpec.describe Eccairs::Occurrence::Attributes::HighestDamage do
  ...
  describe ".attribute_id" do
    it "returns attribute ID of 432" do
      expect(described_class.attribute_id).to eq("432")
    end
  end
  ...
  describe "#build_xml" do
    it "generates valid XML with value" do
      ...
      expect(xml).to include("Highest_Damage")
      expect(xml).to include('attributeId="432"')
    end
    ...
    it "does not generate XML when value is nil" do
      ...
      expect(xml).not_to include("Highest_Damage")
    end
  end
