...
RSpec.describe Eccairs::Occurrence::Attributes::TotalFatalitiesGround do
  ...
  describe ".attribute_id" do
    it "returns attribute ID of 460" do
      expect(described_class.attribute_id).to eq("460")
    end
  end
  ...
  describe "#build_xml" do
    it "generates valid XML with value" do
      ...
      expect(xml).to include("Total_Fatalities_Ground")
      expect(xml).to include('attributeId="460"')
    end
    ...
    it "does not generate XML when value is nil" do
      ...
      expect(xml).not_to include("Total_Fatalities_Ground")
    end
  end
