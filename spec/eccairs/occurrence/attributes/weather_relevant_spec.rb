...
RSpec.describe Eccairs::Occurrence::Attributes::WeatherRelevant do
  ...
  describe ".attribute_id" do
    it "returns attribute ID of 606" do
      expect(described_class.attribute_id).to eq("606")
    end
  end
  ...
  describe "#build_xml" do
    it "generates valid XML with value" do
      ...
      expect(xml).to include("Weather_Relevant")
      expect(xml).to include('attributeId="606"')
    end
    ...
    it "does not generate XML when value is nil" do
      ...
      expect(xml).not_to include("Weather_Relevant")
    end
  end
